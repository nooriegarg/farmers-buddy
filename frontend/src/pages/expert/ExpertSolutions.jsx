...
import { addSolution, getMySolutions, deleteSolution, updateSolution } from "../../services/expertSolutionService"

import { FaLightbulb, FaPlus, FaTrash, FaEdit, FaCheck, FaTimes } from "react-icons/fa"
...
function ExpertSolutions() {
  ...
  const startEdit = (s) => {
    setEditingId(s.id)
    setEditForm({ title: s.title, description: s.description, category: s.category })
  }
  ...
  const handleUpdate = async (id) => {
    try {
      const updated = await updateSolution(id, editForm)
      setSolutions(solutions.map((s) => (s.id === id ? updated : s)))
                  ...
                  {editingId === s.id ? (
                    ...
                    <div className="space-y-3">
                      <input
                        ...
                        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 bg-gray-50"
                      />
                      <textarea
                        rows="3" value={editForm.description}
                        onChange={(e) => setEditForm({ ...editForm, description: e.target.value })}
                        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 bg-gray-50 resize-none"
                      />
                      <select
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex items-center gap-1.5 text-xs font-bold text-green-700 border border-green-200 hover:border-green-400 rounded-lg px-3 py-1.5 transition"
                        >
                          <FaCheck className="text-xs" /> Save
                        </button>
                        <button
                          onClick={cancelEdit}
                          className="flex items-center gap-1.5 text-xs font-bold text-gray-500 border border-gray-200 hover:border-gray-400 rounded-lg px-3 py-1.5 transition"
                        >
                    ...
                    <>
                      <div className="flex items-start justify-between gap-3 mb-3">
                        ...
                        <div className="flex gap-1.5 shrink-0">
                          <button
                            onClick={() => startEdit(s)}
                            className="flex items-center gap-1 text-xs text-violet-600 hover:text-violet-800 border border-violet-200 hover:border-violet-400 rounded-lg px-2.5 py-1.5 transition"
                            title="Edit"
                          >
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 text-xs text-red-500 hover:text-red-700 border border-red-200 hover:border-red-400 rounded-lg px-2.5 py-1.5 transition"
                            title="Delete"
                          >
